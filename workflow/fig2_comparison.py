...
def main():
    ...
    for i, key in enumerate(['Human', 'Zebra finch', 'Turtle']):
        plt.scatter(angles[key][:show], angles_condition[key][:show], label = key, s=100, 
        alpha = np.linspace(1, .1, show), color=colors[i+1])
    ...
    for i, key in enumerate(crossvariance.keys()):
        plt.scatter(crossvariance[key][:show]*100, crossvariance_condition[key][:show]*100, label = organisms[key], s=60, 
        alpha = np.linspace(1, 0, show), color=colors[i+1])
    else:
