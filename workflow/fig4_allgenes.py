...
def main():
    ...
    x = bugeon.obs[['Subtype', 'state_modulation']].groupby("Subtype").mean()['state_modulation'].to_numpy()
    ...
    fig, ax = plt.subplots()
    ax.plot(np.arange(pvals.shape[0])[~significant], corrs[idx][~significant], '-', color='gray', lw=2)
    ax.plot(np.arange(pvals.shape[0])[significant], corrs[idx][significant], 'o', markersize=size, alpha=0.3)
    ...
    np.savetxt(snakemake.output.significant_genes, np.array(avg_expr.columns[pvals_adj < 0.05]), fmt='%s')
