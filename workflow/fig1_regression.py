...
def main(args):
    ...
    # Plot all
    fig, ax = plt.subplots()
    sns.scatterplot(bugeon.obs, x = "tPC1", y = "State modulation", hue="Subclass", 
                ax=ax, linewidth = 0.33, alpha=0.67)
    ax.legend().remove()
    sns.despine()
    ...
    fig.tight_layout()
    plt.savefig(snakemake.output[0], dpi=300) # figdir + 'regression_All.png'
    ...
    for i, subclass in enumerate(subclasses):
        fig, ax = plt.subplots()
        sns.despine()
        sns.scatterplot(bugeon.obs, x = "tPC1", y = "State modulation", hue="Subclass", ax=ax,alpha=0.1,
                        size=1, legend = False, linewidth=.33)
        obs = bugeon[bugeon.obs['Subclass'] == subclass].obs
        sns.scatterplot(obs, x = "tPC1", y = "State modulation", color = sns.color_palette()[i], ax=ax,alpha=.67,
                        size=1, legend = False, linewidth=.33)
...
if __name__=="__main__":
    ...
    parser.add_argument("--n_subsets", default=1000, type=int, 
                        help = "Number of random subsets to draw from all cells")
