...
def main():
    ...
    for i, name in enumerate(['bakken', 'colquitt', 'tosches']):
        plt.bar(x+.8*(i+1), cross_variance[name][:5], label = species[name])
        print(f"{name} tPC1-10: {cross_variance[name][:10].sum()*100:0.1f}% of mouse tPC1")
    plt.legend(handlelength=.75, ncol=2, fontsize=18)
    plt.xticks(x+1., np.arange(5)+1)
    plt.yticks([0, .5, 1])
    sns.despine()
