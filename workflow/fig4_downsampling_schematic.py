...
def main():
    ...
    for name, file in snakemake.input.items():
        ...
        print(f"# counts: {np.median(datasets[name].obs['total_counts'])}, # genes; {np.median(datasets[name].obs['n_genes'])}")
    ...
    plot_loghist(datasets['tasic'].obs['total_counts'], bins=mouse_bins, alpha=.5)
    plot_loghist(datasets['tasic'].obs['total_counts'], bins=mouse_bins, alpha=1.0, 
        histtype="step", color=sns.color_palette()[0])
    ...
    sns.despine()
    plt.text(np.median(datasets['tasic'].obs['total_counts'])*0.3, 1000, 'mouse', color=sns.color_palette()[0])
    plt.text(np.median(datasets['bakken'].obs['total_counts'])*0.3, 1000, 'human', color=sns.color_palette()[1])
    plt.arrow(7.5e5, 500, -6.5e5, 0, head_width=52, head_length=3.2e4, facecolor='black', width=10)
    ...
    plot_loghist(subsampled_counts, bins=bins, histtype='step', color = sns.color_palette()[0], alpha=1.0)
    plt.tight_layout()
