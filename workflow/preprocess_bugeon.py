...
for subdir in subdirs:
    ...
    for experiment in experiments:
        ...
        for recording in recording_folders:
            ...
            subfolder = recording + "/01/" # some also have /02/ but the cell info doesn't match
...
print("% of time in running, stat. sync, and stat. desync state: \
...
print(f"Found {np.isnan(avg_running).sum()} cells not recorded during running state") # 0
print(f"Found {np.isnan(avg_sync).sum()} cells not recorded during synchronized state") # 139
print(f"Found {np.isnan(avg_desync).sum()} cells not recorded during desynchronized state") # 0
...
print("Exclude", sum(df['Subtype'].isin(sparse_types)), "neurons of subtype with < 3 cells") # 8, as listed in paper
...
print(f"{np.sum((np.isnan(bugeon.X)>0).sum(1)>0)} cells with missing counts in {np.sum((np.isnan(bugeon.X)>0).sum(0)>0)} genes")
...
for gene in bugeon.var_names[gene_idx]:
    for subtype in np.unique(bugeon.obs['Subtype']):
        ...
        if idx.sum() > 0:
            bugeon[idx, gene].X = np.nanmedian(bugeon[bugeon.obs['Subtype'] == subtype,gene].X)
