...
def main(args):
    ...
    remaining = np.sort(
        [dataset for dataset in PC_subspace.keys() if dataset != reference]
    )
    ...
    for i, name in enumerate(remaining):
        plt.plot(
            subspace_angles(PC_subspace[name], PC_subspace[reference])[::-1]
            * 180
            / np.pi,
            label=species[name],
            color=colors[i + 1],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exclude_meis2", action="store_const", default=False, const=True
    )
    parser.add_argument(
        "--match_abundance", action="store_const", default=False, const=True
    )
