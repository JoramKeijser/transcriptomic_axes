...
for exp_folder in exp_folders:
    sessions = [
        d
        for d in os.listdir(basedir + exp_folder)
        if os.path.isdir(basedir + exp_folder)
    ]
...
print(
    f"{np.sum(nan_cell)} cells with missing State modulation, {np.sum(~nan_cell)} left"
)
...
sns.despine()
ax.hlines(0, -0.5, 4.5, linestyles=":", color="black", lw=1)
ax.set_ylim([-1.2, 1.2])
ax.set_yticks([-1, 0, 1])
ax.set_xlabel("")
    ...
    for j, subclass2 in enumerate(var_df["Subclass"].cat.categories):
        ...
        if j > i:
            ...
            if j == i + 1:
                # Add it
                if pval < 0.05:
                    plt.plot([i, i, j, j], [y, y + h, y + h, y], lw=0.8, color="black")
                    plt.text(
                        (i + j) * 0.5,
                        y + h,
                        "*",
