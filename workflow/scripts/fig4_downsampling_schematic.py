"""
Schematic to illustrate downsampling
"""
import numpy as np
...
for name, file in snakemake.input.items():
    ...
    median_counts = np.median(datasets[name].obs["total_counts"])
    median_genes = np.median(datasets[name].obs["n_genes"])
    print(f"# counts: {median_counts}, # genes; {median_genes}")
