...
def main():
    ...
    adata = adata[["GABA" in cluster for cluster in adata.obs['cluster_int_sub2']]] # subset GABA
    # Remove clusters that correspond to subcortical mammalian neurons
    adata = adata[[cluster not in ['GABA-7', 'GABA-8', 'GABA-Pre'] for cluster in adata.obs['cluster_int_sub2']]]
    # Annotate clusters with putative mammalian homologues (Colquitt Fig. 4)
    adata.obs['Subclass'] = adata.obs['cluster_int_sub2'].map({'GABA-1-1': "Meis2", "GABA-1-2": "Meis2", 
                                    'GABA-2': "Sst", "GABA-3": "Pvalb", "GABA-4": "Pvalb", 
                                     "GABA-5-1": "Vip", "GABA-5-2": "Vip", "GABA-5-3": "Vip",
                                    "GABA-6": "Lamp5"}).astype("category")
