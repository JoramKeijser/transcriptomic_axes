...
for i, feature in enumerate(['Sequencing depth', 'Number of genes']):
    sns.violinplot(y=feature, data=adata.obs, ax = ax[i], inner = None, linewidth=0, alpha=0.9)
    sns.stripplot(y=feature, data=adata.obs, ax = ax[i], s=2, color='gray', alpha=0.5)
    # One vs the other
ax[0].set_title(dataset[0].upper() + dataset[1:] + " et al.")
ax[2].scatter(adata.obs['Sequencing depth'], adata.obs['Number of genes'], 
        s=1, color='gray', alpha=0.25)
...
df = pd.DataFrame(np.array(stats)[None], 
                    columns = ['name', 'neurons', "depth", "genes"])
