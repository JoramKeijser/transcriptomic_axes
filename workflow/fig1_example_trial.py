...
for subclass in subclass_order:
    ...
    plot_order_subclass_alpha[subclass] = subclass_idx[
        np.argsort(gaba_ttypes[subclass_idx])[::-1]
    ]
...
cmaps = [LinearSegmentedColormap.from_list("", ["white", colors[i]]) for i in range(5)]
...
gs = GridSpec(
    7, 2, height_ratios=height_ratios, hspace=0.1, wspace=0.1, left=0.25, right=0.9
)
...
for c, phase in enumerate([0, 2]):
    ...
    if c == 0:
        ...
        ax0.text(running_times[i0] - 30, 30, "State", fontsize=FONTSIZE, color="black")
    ax0.plot(
        running_times[i0:i1],
...
for r, subclass in enumerate(subclass_order):
    ...
    for c, phase in enumerate([2, 0]):
        ...
        if c == 0:
            ax.set_ylabel(
                subclass, rotation=0, fontsize=FONTSIZE, labelpad=20, color=colors[r]
            )
        ...
        if (r == 4) & (c == 0):
            Y0 = 0
            ax.hlines(
                -10, Y0, Y0 + 10 / np.diff(states_times)[0], lw=LINEWIDTH, color="black"
            )
