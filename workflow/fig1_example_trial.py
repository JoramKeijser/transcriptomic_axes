...
def main():
    ...
    desync = gaba_activity[states==1].mean(0)
    ...
    eye_size = np.squeeze(np.load(recording + "/01/" + "eye.size.npy"))
    ...
    speed = np.convolve(running_speed, running_kernel, mode='same')

    # Where to start and end?
    to_plot = states_times[states==2][np.where(np.diff(np.where(states == 2)[0])  > 1)[0]]
    end_of_first = to_plot[0]
    end_of_first_index = np.where(running_times >= end_of_first)[0][0]
    start_of_first_index = np.where(states==2)[0][0]
    #start_of_first = states_times[start_of_first_index]of
    start_of_first = end_of_first - 40
    start_of_first_index =  np.where(running_times >= start_of_first)[0][0]
    ...
    n_ec = ec_activity.shape[1]
    ten_pct = int(.10*n_ec)
    ec_sort = np.argsort(pca.components_[0])

    # Plot maps
    colors = sns.color_palette("colorblind", 5)
    cmaps = [ matplotlib.colors.LinearSegmentedColormap.from_list("", ["white", colors[i]]) for i in range(5)]
    ...
    for r, subclass in enumerate(subclass_order):
        ...
        for c, phase in enumerate([2,0]):
            ...
            if (c==0):
                ax.set_ylabel(subclass,rotation=0, fontsize=fontsize, labelpad=20, color=colors[r])
            #if  (r==2):
            #    ax.set_title(phase_names[phase], fontsize=fontsize)
            ...
            if (r==4) & (c == 0):
                y0 = 0
                ax.hlines(-10, y0, y0+10 / np.diff(states_times)[0], lw=lw, color='black')
                ax.text(1, -35,  "10 s", fontsize=fontsize_small)
            if (r==0) & (c==1):
                x = i1-i0-1
                dy = 25
                ax.vlines(x, len(idx)-dy, len(idx)-dy+10, lw=lw, color='black')
                ax.text(x+15, len(idx)-37, "10 neurons", fontsize=fontsize_small, rotation=90,  horizontalalignment='center')
